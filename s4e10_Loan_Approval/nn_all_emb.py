...
class EmbDataset(Dataset):
    ...
    def __getitem__(self,idx, batch_size):
        ...
        return [cat_val, y] #[num_val, cat_val, y]
...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model):
        ...
        if self.best_score is None:
            ...
            torch.save(model.state_dict(), 'data/models/best.pt')
    ...
    def load_best_model(self):
        model2 = Model(meta_data, 0.1, [256, 256, 64], 3*[0.1]).to(DEVICE)
        model_data = torch.load('data/models/best.pt', weights_only=False)
...
df_train = pd.concat([df_train, df_org]).reset_index(drop=True)
df_train = df_train.loc[~df_train.duplicated(subset=ORD+CATS),:].reset_index(drop=True)
...
for col in ORD:
    ...
    df_comb[col] = to_rank(df_comb[col])


# df_train = df_train.drop(columns=ORD)
# df_test = df_test.drop(columns=ORD)
df_train = df_comb.iloc[:len_train,:]
print(df_train.shape)
df_test = df_comb.iloc[len_train:,:]
print(df_test.shape)
CATS = CATS # + ORD #add_col
#ORD = []
...
# for col in CATS:
#     print(f'{col} number of groups {len(df_train[col].unique())}')
...
test_ord = test_ord.drop(columns='loan_status')
...
for i, (train_idx, val_idx) in enumerate(kf.split(train_ord, df_y)):
    ...
    for epoch in range(EPOCHS):
        ...
        validation_loss, validation_aucroc, oof = \
                    valid(model, validloader, DEVICE, oof, val_idx)
        ...
        early_stopping(validation_aucroc, model) #validation_loss, model)   
        if early_stopping.early_stop:
            print("Eary stopping")
            break
    
    # Get best model, and recalc best aucroc, and save
    model = early_stopping.load_best_model()
    print('Recalc best aucroc')
    validation_loss, validation_aucroc, oof = \
                    valid(model, validloader, DEVICE, oof, val_idx)
...
df_sub['loan_status'] = pred
df_sub.to_csv('submissions/nn_38.csv', index=False)
df_sub.to_csv('ensembling2/38_pred.csv')

df_check = pd.read_csv('submissions/nn_38.csv') # Get index 
display(df_check)


df_oof['loan_status'] = oof
display(df_oof)
df_oof.to_csv('ensembling2/38_oof.csv')
